...
namespace Graball.General.Text.Formatter
{
    ...
    /// </summary>
    public class FormatHtmlToXml : FormatNulled
    {
        /// <summary>
        /// <para>Construtor.</para>
        /// </summary>
        public FormatHtmlToXml() : base() { }
        ...
        public override string Format(string format, object arg, IFormatProvider formatProvider)
        {
            ...
            foreach (Match match1 in Regex.Matches(result.ToString(), @"(?<=\<[a-z]+\s[^\>]*[a-z0-9]*\=)\""[^\>]*?\""(?=[\>\s])", RegexOptions.IgnoreCase | RegexOptions.Singleline))
            {
                if (match1.Value.Length > 2 && match1.Value.IndexOf("\"", 1) < match1.Value.Length - 1)
                {
